...
const { scenarios, ...modules } = allModules;

export default function startMirage(...scenarioNames) {
  const options = Object.assign(modules, { environment, baseConfig });
  const server = new Mirage(options);
  ...
  scenarioNames.filter(Boolean).forEach((scenarioName) => {
    const scenario = scenarios[camelize(scenarioName)];
